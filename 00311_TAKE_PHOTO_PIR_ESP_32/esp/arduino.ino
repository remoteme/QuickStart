...
#define DEVICE_ID 3
#define DEVICE_NAME "Photo taker"
#define TOKEN "~155_p7hkXSvK3bwt"
...
#define PIR_PIN 12 // PIR on GPIO16
#define LED_PIN 4  // GPIO4 is the Flash LED
#define repeatingSendTimeoutSeconds 10
...
void takePhoto(){
    ...
    remoteMe.setFileContent(4,"photos/photo.jpg",fb->len,fb->buf);
    Serial.printf(" buff size %d \n",fb->len);
    Serial.println("sent");
}
void onUserMessage(uint16_t senderDeviceId, uint16_t dataSize, uint8_t *data){
  ...
  static long lastSend=0;
    remoteMe.loop();
     ...
     if (digitalRead(PIR_PIN) && ((lastSend==0) || (lastSend+repeatingSendTimeoutSeconds*1000<millis()))) {
       takePhoto();
       lastSend=millis();
      remoteMe.sendPushNotificationMessage(4, "Move appears", "Move", "badge.png", "icon192.png", "photos/photo.jpg");
